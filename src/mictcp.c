...
// Taux des paquets perdus et abandonnes. Si LOSS_RATE_VALUE = 10, 10% des paquets perdus ne seront pas renvoyes. donc on renvoie un paquet sur 10
#define LOSS_RATE_VALUE 50
...
// Valeur a 1 paquet renvoye, paquet a 0 paquet ok
int tabLastPaquet[WINDOW_SIZE] = {0};
...
int mic_tcp_socket(start_mode sm)
{
    ...
    ourSocketTab[result].socket = ourSocket;
    ourSocketTab[result].num_seq_local = 0;
    ourSocketTab[result].num_seq_distant = 0;
    id_sock = result;
...
int mic_tcp_send(int mic_sock, char *mesg, int mesg_size)
{
    ...
    while (1)
    {
        ...
        if ((pdu_to_rcv.header.ack_num == ourSocketTab[mic_sock].num_seq_local) && (pdu_to_rcv.header.ack == 1) && (size_pdu_rcv != -1))
        {
            nb_send_paquets++;
            break;
        }
    ...
    compteurTab = (compteurTab + 1) % WINDOW_SIZE;

    return size_octets_sent;
}

/**
 * @brief cette fonction nous dit si une perte peut etre acceptee ou pas
 *
 * @return int 0 si on accepte pas la perte 1 si on l'accepte
 */
int accept_loss(){
    int sum = 0;
    ...
    if(percentageLoss < LOSS_RATE_VALUE){
        return 1;
    }
...
int mic_tcp_recv(int socket, char *mesg, int max_mesg_size)
{
    ...
    if (DEBUG_MODE)
    {
        if (delivered_size != -1)
        {
            ...
            printf("[INFO] Nombre de paquets lu dans le buffer : %d\n", nb_rcv_paquets);
        }
        else
...
void process_received_PDU(mic_tcp_pdu pdu, mic_tcp_sock_addr addr)
{
    ...
    case ESTABLISHED:
        ...
        // Si c'est le bon num de seq on envoie le ack et met a jour le num ack
        ...
        // Si c'est pas le bon num de seq on envoie le ack sans mettre a jour le num ack
        else
        {
            mic_tcp_pdu pdu_ack;
            initialise_to_null_pdu(&pdu_ack);
            ...
            if (DEBUG_MODE)
            {
                print_mic_tcp_pdu_infos(pdu_ack, "PDU ACK CREATED MAIS PAS BON NUM SEQ:");
            }

            int sent_size;
            if ((sent_size = IP_send(pdu_ack, addr) == -1))
            {
                printf("[ERROR] Echec d'envoie du PDU ACK MAIS PAS BON NUM SEQ\n");
                return;
            }
        }
        break;
...
/**
 * @brief Affiche les informations d'un PDU
 * 
 * @param pdu pdu a afficher les infos
 * @param text text d'entete
 */
